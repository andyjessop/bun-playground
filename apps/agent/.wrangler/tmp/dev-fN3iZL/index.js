...
// .wrangler/tmp/bundle-UnUIT3/checked-fetch.js
...
var Pipeline = class {
  ...
  async run(initialInput, options = {}) {
    ...
    const runPipe = async (input) => {
      let result2 = await this.inputNode(input);
      for (const operator of this.operators) {
        result2 = await operator(result2);
      }
      return result2;
    };
...
var Stream = class {
  ...
  static fromSSEResponse(response2, controller) {
    let consumed = false;
      ...
      try {
        for await (const sse of _iterSSEMessages(response2, controller)) {
          ...
          if (sse.event === "error") {
            ...
            throw APIError.generate(void 0, errJSON, errMessage, createResponseHeaders(response2.headers));
          }
        }
        done = true;
      } catch (e) {
...
async function* _iterSSEMessages(response2, controller) {
  ...
  const lineDecoder = new LineDecoder();
  const iter = readableStreamAsyncIterable(response2.body);
...
async function defaultParseResponse(props) {
  const { response: response2 } = props;
  if (props.options.stream) {
    debug("response", response2.status, response2.url, response2.headers, response2.body);
    if (props.options.__streamClass) {
      return props.options.__streamClass.fromSSEResponse(response2, props.controller);
    }
    return Stream.fromSSEResponse(response2, props.controller);
  }
  ...
  if (isJSON) {
    ...
    debug("response", response2.status, response2.url, response2.headers, json);
    return json;
  }
  const text = await response2.text();
  debug("response", response2.status, response2.url, response2.headers, text);
  return text;
}
...
var APIClient = class {
  ...
  async makeRequest(optionsInput, retriesRemaining) {
    ...
    const response2 = await this.fetchWithTimeout(url, req, timeout, controller).catch(castToError);
    if (response2 instanceof Error) {
      ...
      if (response2.name === "AbortError") {
        throw new APIConnectionTimeoutError();
      }
      throw new APIConnectionError({ cause: response2 });
    }
    const responseHeaders = createResponseHeaders(response2.headers);
    if (!response2.ok) {
      if (retriesRemaining && this.shouldRetry(response2)) {
        const retryMessage2 = `retrying, ${retriesRemaining} attempts remaining`;
        debug(`response (error; ${retryMessage2})`, response2.status, url, responseHeaders);
        return this.retryRequest(options, retriesRemaining, responseHeaders);
      }
      const errText = await response2.text().catch((e) => castToError(e).message);
      const errJSON = safeJSON(errText);
      const errMessage = errJSON ? void 0 : errText;
      const retryMessage = retriesRemaining ? `(error; no more retries left)` : `(error; not retryable)`;
      debug(`response (error; ${retryMessage})`, response2.status, url, responseHeaders, errMessage);
      const err = this.makeStatusError(response2.status, errJSON, errMessage, responseHeaders);
      throw err;
    }
    return { response: response2, options, controller };
  }
...
var AbstractPage = class {
  constructor(client, response2, body, options) {
...
app.post("/run", async (c) => {
  ...
  const result = await new Pipeline().input((prompt2) => prompt2).pipe(
    async (prompt2) => {
    ...
    (prompt2) => Boolean(prompt2.length)
  ).run(prompt);
  return json(result);
});
...
// .wrangler/tmp/bundle-UnUIT3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-UnUIT3/middleware-loader.entry.ts
